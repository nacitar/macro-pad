...
echo "Flashing (AUTOMATION_MODE=${AUTOMATION_MODE_VALUE:-1}, MOUSEKEY_ENABLE=${MOUSEKEY_ENABLE_VALUE:-no}, BRIGHTNESS=${BRIGHTNESS:-50})..."
uv run qmk flash -kb "$KB" -km "$KM" \
    -e "AUTOMATION_MODE=${AUTOMATION_MODE_VALUE:-1}" \
    -e "MOUSEKEY_ENABLE=${MOUSEKEY_ENABLE_VALUE:-no}" \
    -e "STATUS_LED_BRIGHTNESS=${BRIGHTNESS:-50}"
